...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	//int ht = grid.size();
	//int wt = grid[0].size();
vector<float>::size_type ht = grid.size();
vector<float>::size_type wt = grid.size();
	for (int i= 0;i<ht; i++)
	{
		for (int j=0;j<wt;j++)
		{
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	float centerprobab = 1.0 - blurring;
	float cornerprobab = blurring/12.0;
	float adjacentprobab = blurring/6.0;

    vector <vector <float> > window = {
		{cornerprobab, adjacentprobab, cornerprobab},
	{adjacentprobab, centerprobab, adjacentprobab},
	{cornerprobab,adjacentprobab,cornerprobab}
	};
	...
	for (int i = 0;i < ht;i++)
	{
		for (int j = 0;j < wt;j++)
		{
			...
			for (int dx = -1; dx < 2; dx++)
			{
				...
				{
					int update_i = (i + dy + ht) % ht;
					int update_j = (j + dx + wt) % wt;
					float mult = window[dx + 1][dy + 1];
					newGrid[update_i][update_j] += grid_val * mult;
			}
		}
	}
}
